...
- **不拦**免责式否定(「这不是买入信号」「不该读成贪婪就卖」——含禁词但是正当否定,同 honesty-guard 教训 [[honesty-guard-test-disclaimers]])。
  实现:守门只列**明确操作指令**短语(建议买/卖·赶紧买/卖·可以买入/卖出·加仓/清仓/满仓·止损·抄底吧),
  **故意不含含糊的「该买/该卖」**(它们出现在 应该/不该/就该 里、否定常远离禁词,裸词匹配必误伤免责句);
  紧邻否定字(不/别/非/勿/绝)的命中放行。「该买/该卖」的拦截交给 prompt 铁律,不进守门。
